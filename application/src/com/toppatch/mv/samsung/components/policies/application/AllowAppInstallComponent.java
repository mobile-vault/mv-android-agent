...
public class AllowAppInstallComponent extends Component {
	...
	public void execute(JSONObject data) {
		if(data!=null){
			...
			try {
				boolean enable = data.getBoolean(Constants.APP_ALLOW_INSTALL);
				EnterpriseDeviceManager edm = getEdm();
				if(edm!=null){
					try{
						ApplicationPolicy applicationPolicy = edm.getApplicationPolicy();
						if(!enable) applicationPolicy.setApplicationInstallationMode(ApplicationPolicy.APPLICATION_INSTALLATION_MODE_DISALLOW);
						else applicationPolicy.setApplicationInstallationMode(ApplicationPolicy.APPLICATION_INSTALLATION_MODE_ALLOW);
					}catch(Exception e){
