package com.toppatch.mv.samsung.components.policies;
...
import com.toppatch.mv.samsung.components.policies.restriction.WifiComponent;
...
import com.toppatch.mv.samsung.components.policies.wifi.WifiUpdateComponent;
...
public class PolicyControllerComponent extends Component {
	...
	public void execute(JSONObject data) {
		...
		if(application!=null) {
			...
			JSONObject playstoreEnable = application.optJSONObject("playstore_enable");
			ExecutionHelper.executePlayStoreEnable(mContext, playstoreEnable);

			JSONObject enableRecording = application.optJSONObject("enable_recording");
			ExecutionHelper.executeEnableRecording(mContext, enableRecording);

			JSONObject browserSettings = application.optJSONObject("browser_settings");
			ExecutionHelper.executeBrowserSettings(mContext, browserSettings);

			JSONArray installedApps = application.optJSONArray("installed_apps");
			ExecutionHelper.executeInstalledApps(mContext, installedApps);

			JSONArray removedApps = application.optJSONArray("removed_apps");
			ExecutionHelper.executeRemovedApps(mContext, removedApps);

			JSONArray blackListedApps = application.optJSONArray("blacklisted_apps");
			ExecutionHelper.executeBlackListedApps(mContext, blackListedApps);	
			
			JSONObject youTubeEnable = application.optJSONObject("youtube_enable");
			ExecutionHelper.executeYouTubeEnable(mContext,youTubeEnable);	
			
			JSONObject allowAppInstall = application.optJSONObject("allow_app_installation");
			ExecutionHelper.executeAllowAppInstall(mContext,allowAppInstall);	
			/*
			 * Camera
		...
		if(hardware!=null)Log.d(TAG, "Got hardware as "+hardware.toString());
		if(hardware!=null){

			JSONObject cameraEnable = hardware.optJSONObject(Constants.PLUG_HARDWARE_CAMERA);
			ExecutionHelper.executeCameraEnable(mContext, cameraEnable);

			JSONObject externalStorageEncryptionEnable = hardware.optJSONObject(Constants.PLUG_HARDWARE_EXTERNAL_STORAGE_ENCRYPTION);
			ExecutionHelper.executeExternalStorageEncryptionEnable(mContext, externalStorageEncryptionEnable);

			JSONObject internalStorageEncryptionEnable = hardware.optJSONObject(Constants.PLUG_HARDWARE_INTERNAL_STORAGE_ENCRYPTION);
			ExecutionHelper.executeInternalStorageEncryption(mContext, internalStorageEncryptionEnable);

			JSONObject microphoneEnable = hardware.optJSONObject(Constants.PLUG_HARDWARE_MIC_STATE);
			ExecutionHelper.executeMicrophoneState(mContext, microphoneEnable);

			JSONObject androidBeamEnable = hardware.optJSONObject(Constants.PLUG_HARDWARE_ANDROID_BEAM);
			ExecutionHelper.executeAndroidBeam(mContext, androidBeamEnable);
		}else{
		...
		if(settings!=null){
			JSONObject backgroundDataEnable= settings.optJSONObject(Constants.PLUG_SETTINGS_BACKGROUND_DATA);
			ExecutionHelper.executeBackgroundEnable(mContext, backgroundDataEnable);

			JSONObject wifiEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_WIFI);
			ExecutionHelper.executeWifiEnable(mContext, wifiEnable);

			JSONObject backupEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_BACKUP);
			ExecutionHelper.executeBackupEnable(mContext, backupEnable);

			JSONObject clipboardEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_CLIPBOARD);
			ExecutionHelper.executeClipboardEnable(mContext, clipboardEnable);

			JSONObject googleCrashReportEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_GOOGLE_CRASH_REPORT);
			ExecutionHelper.executeGoogleCrashReportEnable(mContext, googleCrashReportEnable);

			JSONObject dataWhileRoamingEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_DATA_WHILE_ROAMING);
			ExecutionHelper.executeDataWhileRoamingEnable(mContext, dataWhileRoamingEnable);

			JSONObject pushWhileRoamingEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_PUSH_WHILE_ROAMING);
			ExecutionHelper.executePushWhileRoamingEnable(mContext, pushWhileRoamingEnable);

			JSONObject syncWhileRoamingEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_SYNC_WHILE_ROAMING);
			ExecutionHelper.executeSyncWhileRoamingEnable(mContext, syncWhileRoamingEnable);

			JSONObject voiceCallsWhileRoamingEnable = settings.optJSONObject(Constants.PLUG_SETTINGS_VOICE_CALLS_WHILE_ROAMING);
			ExecutionHelper.executeVoiceCallsWhileRoamingEnable(mContext, voiceCallsWhileRoamingEnable);
		}else{
		...
		if(wifi!=null){
			new WifiAddComponent(mContext).execute(wifi);
		}else{
		...
		if(vpn!=null){
			new VPNAddComponent(mContext).execute(vpn);
		}else{
		...
		if (access!=null){
			try {
				JSONObject changeSettingsEnable = access.getJSONObject(Constants.PLUG_ACCESS_CHANGE_SETTINGS);
				new ChangeSettingsComponent(mContext).execute(changeSettingsEnable);
			} catch (JSONException e) {
				Log.d(TAG, "No change settings sent "+Constants.PLUG_ACCESS_CHANGE_SETTINGS );
			}
			
			try {
				JSONObject screenShotEnable = access.getJSONObject(Constants.PLUG_ACCESS_CAPTURE_ENABLE);
				new AllowScreenShotComponent(mContext).execute(screenShotEnable);
			} catch (JSONException e) {
				Log.d(TAG,"No "+Constants.PLUG_ACCESS_CAPTURE_ENABLE+" sent");
			}
			
			try {
				JSONObject changeAdminEnable = access.getJSONObject(Constants.PLUG_ACCESS_CHANGE_ADMIN);
				new AllowRemoveAdminComponent(mContext).execute(changeAdminEnable);
			} catch (JSONException e) {
				Log.d(TAG,"No "+Constants.PLUG_ACCESS_CHANGE_ADMIN+" sent");
			}
			
			try {
				JSONObject usbDebuggingEnable = access.getJSONObject(Constants.PLUG_ACCESS_ENABLE_USB);
				new USBDebuggingComponent(mContext).execute(usbDebuggingEnable);
			} catch (JSONException e) {
				Log.d(TAG, "No "+Constants.PLUG_ACCESS_ENABLE_USB+" sent");
			}
			
			try {
				JSONObject factoryResetEnable = access.getJSONObject(Constants.PLUG_ACCESS_FACTORY_RESET);
				new AllowFactoryResetComponent(mContext).execute(factoryResetEnable);
			} catch (JSONException e) {
		...
		if (bluetooth!=null){
			try {
				JSONObject bluetoothStatus = bluetooth.getJSONObject(Constants.PLUG_BT_STATUS);
				new BluetoothPowerStateComponent(mContext).execute(bluetoothStatus);
				new BluetoothStateComponent(mContext).execute(bluetoothStatus);
			} catch (JSONException e) {
				Log.d(TAG, "No enable settings sent "+Constants.PLUG_BT_STATUS);
			}
		
			try {
				JSONArray blackListedPairing = bluetooth.getJSONArray(Constants.PLUG_BT_BLACK_LISTED);
				JSONObject jsonObject = new JSONObject();
				jsonObject.put(Constants.BLUETOOTH_LIST,blackListedPairing);
				new BluetoothAddToBlackListComponent(mContext).execute(jsonObject);
			} catch (JSONException e) {
				Log.d(TAG, "No blackList pairings "+Constants.PLUG_BT_BLACK_LISTED);
			}
			try {
				JSONArray whiteListedPairing = bluetooth.getJSONArray(Constants.PLUG_BT_WHITE_LISTED);
				JSONObject jsonObject = new JSONObject();
				jsonObject.put(Constants.BLUETOOTH_LIST,whiteListedPairing);
				new BluetoothAddToWhiteListComponent(mContext).execute(jsonObject);
			} catch (JSONException e) {
...
class ExecutionHelper{
	...
	public static void executePlayStoreEnable(Context context,JSONObject playStore){
		if(playStore!=null){
	...
	public static void executeAllowAppInstall(Context context,JSONObject appInstall){
		if(appInstall!=null){
	...
	public static void executeEnableRecording(Context context,JSONObject recording){
		if(recording!=null){
	...
	public static void executeBrowserSettings(Context context,JSONObject browser){
		if(browser!=null){
			try {
				boolean enableAutofill = browser.getBoolean("enable_autofill");
				JSONObject jsonObject = new JSONObject();
				jsonObject.put(Constants.BROWSER_AUTOFILL_ENABLE, enableAutofill);
			...
			try {
				boolean enableJavaScript = browser.getBoolean("enable_javascript");
				JSONObject jsonObject = new JSONObject();
				jsonObject.put(Constants.BROWSER_JAVASCRIPT_ENABLE, enableJavaScript);
			...
			try {
				boolean forceFraudWarnings = browser.getBoolean("force_fraud_warnings");
				JSONObject jsonObject = new JSONObject();
				jsonObject.put(Constants.BROWSER_FORCE_FRAUD_WARNING_ENABLE, forceFraudWarnings);
			...
			try {
				boolean enableHttpProxy = browser.getBoolean("enable_http_proxy");
	...
	public static void executeRemovedApps(Context context,JSONArray apps){
		if(apps!=null && apps.length()>0){
			...
			JSONObject jsonObject = new JSONObject();
			try {
				jsonObject.put(Constants.APP_ANDROID_UNINSTALL, apps.toString());
	...
	public static void executeExternalStorageEncryptionEnable(Context context,JSONObject obj){
		if(obj!=null){
	...
	public static void executeInternalStorageEncryption(Context context,JSONObject obj){
		if(obj!=null){
	...
	public static void executeGoogleCrashReportEnable(Context context,JSONObject obj){
	...
	public static void executeDataWhileRoamingEnable(Context context,JSONObject obj){
	...
	public static void executePushWhileRoamingEnable(Context context,JSONObject obj){
	...
	public static void executeSyncWhileRoamingEnable(Context context,JSONObject obj){
	...
	public static void executeVoiceCallsWhileRoamingEnable(Context context,JSONObject obj){
